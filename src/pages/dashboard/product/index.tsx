...
interface CremosinhoProps {
  allCremosinho: ICremosinhoType[];
}

const Cremosinho: FC<CremosinhoProps> = ({ allCremosinho }) => {
  ...
  const [cremosinho, setCremosinho] = useState<ICremosinhoType[]>(allCremosinho);
  ...
  const rows = cremosinho.map((element) => (
    <tr key={element.id_cremosinho}>
      ...
      <td className={styles.tableFlex}>
        <ActionIcon size={20} color="blue" onClick={()=>modalUpdate(element)}>
          <IconEdit />
        </ActionIcon>
        <ActionIcon
  ...
  const modalUpdate = (data:ICremosinho) => openModal({
    title: "Editar Produto",
    centered: true,
    radius: "md",
    children: (
      <AddCremosinho onClose={closeAllModals} onSubmit={updateProduct} value={data}/>
    ),
  });
